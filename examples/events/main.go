...
func main() {
	...
	app.OnServe().BindFunc(func(e *turbine.ServeEvent) error {
		e.Router.POST("/request/{id}", func(re *turbine.RequestEvent) error {
		...
		e.Router.GET("/request/{id}/status", func(re *turbine.RequestEvent) error {
			id := re.Request.PathValue("id")
		...
		e.Router.POST("/request/{id}/approve", func(re *turbine.RequestEvent) error {
			id := re.Request.PathValue("id")
